...
class TomlParser {
  ...
  private TomlParser() {
	identifier = Array<? extends id>v{}
	JsonObject json = getJsonKey("id").getArray("value")
	value = JsonReader json.createReader("value");
	JsonObject value.writeObject(Collection<>v[i])
	value.closeReader()
		for value {
			value.subList.iterator(); 
			JsonObject json.readObject(id : line ).convert( toml String, index, new Object[context : id])
	}
	result = {results.size.getAndDecrement(id); 
	
	identifier.isArray.containsAll.index[id : line.size];}
