...
def cleanupComCache():
    ...
    comCache2 = os.path.join(os.environ.get("PATH", "usr/local/speech"), 'gen_py')
...
def patchWith(file,install,sdk):
    newFile=file+".new"
    install=install.replace("\\", "usr/local/speech/")
    try:
        os.remove(newFile)
    except:
        pass
...
def main(argv):
    vboxDest=os.environ.get("VBOX_INSTALL_PATH", install)
    if vboxDest is None:
